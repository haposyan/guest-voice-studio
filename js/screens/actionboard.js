        ...
        return `<div class="kanban-col">
          ...
          ${list.map((t) => {
            ...
            return `<div class="kanban-card ${overdue ? "overdue" : ""}" data-task="${t.id}">
              ...
              <div class="meta">${escapeHtml(db.storeName(t.storeId))} ・ ${escapeHtml(t.assignee || "未割当")}</div>
  ...
  function renderSelected() {
    ...
    selectedEl.innerHTML = recs.map((r) => `
      <div class="comment-item">
        <div class="meta">${escapeHtml(db.storeName(r.storeId))} / ${r.date} / ★${r.rating ?? "-"}
...
function openTaskForm(root, prefill) {
  ...
  openModal(`
    ...
    <div class="field-row">
      <div class="field"><label>対象拠点</label><select id="fStore">${stores.map((s) => `<option value="${s.id}" ${prefill?.storeId===s.id?"selected":""}>${escapeHtml(s.name)}</option>`).join("")}</select></div>
  ...
  `, { width: 620, onMount: (r) => {
    ...
    r.querySelector("#saveTask").onclick = () => {
      ...
      const task = {
        id: db.uid("task"),
        storeId: r.querySelector("#fStore").value,
...
function openTaskDetail(taskId, root) {
  ...
  openModal(`
    ...
    <div data-pane="basic">
      <div class="field-row">
        <div class="field"><label>拠点</label><div>${escapeHtml(db.storeName(task.storeId))}</div></div>
