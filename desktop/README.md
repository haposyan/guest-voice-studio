...
- **起動画面にロゴとタグラインを追加**：「お客様の声を確認するツールです」の
  一言を添えました。ロゴは本物の休暇村ロゴ素材を持っていないため、現状は
  ブランドカラーで作ったプレースホルダーです（下記「既知の制限」参照）。
...
- **起動画面のロゴはプレースホルダーです**：休暇村の正式なロゴ素材（画像ファイル）を
  お預かりしていないため、ブランドカラーで作った仮のバッジ（「休暇村」の文字）を
  表示しています。正式なロゴ画像（PNG等）をいただければ、`MainWindow.xaml` の
  該当箇所を画像に差し替えます。
