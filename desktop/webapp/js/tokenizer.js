...
// Derives a positive/negative/neutral label from a word's rating-band tally.
// An explicit user correction (ANL-07) always wins over the derived guess.
export function classifySentiment(entry, overrides) {
  const override = overrides?.[entry.word];
  if (override) return override;
  const { negCount, posCount, midCount } = entry;
  if (negCount === 0 && posCount === 0 && midCount === 0) return "neutral";
  if (negCount > posCount && negCount >= midCount) return "negative";
  if (posCount > negCount && posCount >= midCount) return "positive";
  return "neutral";
}

// records: array of {comment, rating, storeId, itemId, ...}
// returns [{word, count, ratePer100, negCount, posCount, midCount, sentiment, sentimentIsOverridden}]
export function computeWordFrequencies(records, excludedWords, ratingBands, sentimentOverrides) {
  const freq = new Map();
  let commentCount = 0;
  records.forEach((rec) => {
  ...
  const overrides = sentimentOverrides || {};
  const list = [...freq.values()].map((e) => ({
    ...e,
    ratePer100: commentCount ? +(e.count / commentCount * 100).toFixed(1) : 0,
    sentiment: classifySentiment(e, overrides),
    sentimentIsOverridden: !!overrides[e.word],
  }));
